...
int carrega_configuracio(FILE * fit)
{
	...
	while(fscanf(fit, "%f %f %f %f\n", &pos_f[0], &pos_c[0], &vel_f[0], &vel_c[0])==5){	/* pilota */
...
void comprovar_bloc(int f, int c)
{
	...
	if (quin == BLKCHAR || quin == FRNTCHAR) {
		...
		if (quin == BLKCHAR){
			...
			vel_f[ind] = 0.9;
			vel_c[ind] = 0.9;
			pthread_create(&tid[ind], NULL, mou_pilota, (intptr_t) ind);
			ind++;
			num_pilotes++;
		}
	}
...
int main(int n_args, char *ll_args[])
{
	...
	if (nblocs == 0){
		sprintf(Temps, "YOU WIN! Temps: %d:%d", minuts, segons);
		mostra_final(Temps);
	}
	else{
		sprintf(Temps, "GAME OVER! Temps: %d:%d", minuts, segons);
		mostra_final(Temps);
	}
